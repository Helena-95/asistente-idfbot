...
def extraer_texto_url(url):
    try:
        ...
        # Limpiamos la web de scripts y estilos
...
# Paso 2: Si está autenticado, mostrar los selectores de modo
    ...
    # Añadimos el texto de la URL si existe
    if extra_text:
        prompt_parts.append({"text": f"Información adicional de la web: {extra_text}"})
    
    # Añadimos los archivos PDF
    ...
    try:
        ...
        st.session_state.chat = model.start_chat(
            history=[{"role": "user", "parts": prompt_parts},
                     {"role": "model", "parts": [{"text": "He procesado el PDF y la información de la URL. ¿Cómo puedo ayudarte?"}]}]
        )
    except Exception as e:
    ...
    if st.session_state.mode == "Guía Docente":
        st.write("Preguntas sobre la **Guía Docente** y la web oficial.")
        
        if st.session_state.chat is None:
            # Pon aquí la URL que quieras que el asistente lea
            url_adicional = "https://www.tu-universidad.es/guia-asignatura" 
            
            with st.spinner("Leyendo información de la web..."):
                texto_web = extraer_texto_url(url_adicional)
            
            iniciar_chat(
                files_to_load=["guia_docente.pdf"], 
                system_prompt={"text": "Responde usando el PDF y la información de la web proporcionada. Si hay conflictos, prioriza el PDF."},
                extra_text=texto_web
            )
            
            if not st.session_state.histories["Guía Docente"]:
                 st.session_state.histories["Guía Docente"].append({
                     "role": "assistant", 
                     "content": "Hola, he analizado la guía PDF y la web. ¿Qué duda tienes?"
                 })
