...
def load_pdf_block(pdf_path):
    ...
    return {
        ...
        "cache_control": {"type": "ephemeral"},  # Claude cachea el PDF entre llamadas
    }
...
def iniciar_modo(mode_key, files_to_load, system_prompt, extra_text=""):
    ...
    st.session_state.chat = True  # Marca el modo como inicializado
...
def enviar_a_claude(mode_key, user_text):
    ...
    # En el primer mensaje adjuntamos los PDFs con cache_control ephemeral.
    # Claude los cachea automáticamente: los mensajes siguientes son más rápidos y baratos.
    if not api_msgs:
...
_defaults = {
    ...
    "mode": None,
    "sub_mode": None,
    "chat": None,           # None = no inicializado; True = listo
    "histories": {},
    "api_messages": {},     # {mode_key: lista de mensajes API}
    "system_prompts": {},   # {mode_key: system prompt string}
    "pdf_blocks": {},       # {mode_key: lista de bloques PDF}
}
...
# ============================================================
    ...
    def change_mode():
        st.session_state.chat = None   # Fuerza reinicialización al cambiar de modo
    ...
    # ── RESOLVER EJERCICIOS ───────────────────────────────────────────────────
        ...
        if tipo_ejercicio == "Proyecto de Inversión":
            ...
            excel_file = st.file_uploader(
                ...
                type=["xlsx", "xls"],
                help="El asistente leerá el Excel directamente: no necesitas copiar los datos a mano."
            )
            if excel_file:
